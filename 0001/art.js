// Referenced from  https://levelup.gitconnected.com/make-digital-art-with-javascript-643315bfef0d

let canvasWidth = document.querySelector('.art').clientWidth;
let canvasHeight = document.querySelector('.art').clientHeight;
  ...
  p.setup = function() {
    ...
    for (let i = 0; i < 5; i++) {
      drawFunctions.forEach(draw => {
        let x = p.random(0,canvasWidth);
        let y = p.random(0,canvasHeight);
